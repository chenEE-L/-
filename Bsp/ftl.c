...
//返回值:0,正常
//    其他,失败
unsigned short lut[4096]={0};
u8 FTL_Init(void)
{
    u8 temp;
	fsmc_nand_init();
    //if(NAND_Init())return 1;									//初始化NAND FLASH
	//if(nand_dev.lut)myfree(nand_dev.lut);
	nand_dev.lut=lut;//mymalloc((char)(nand_dev.block_totalnum)*2); 	//给LUT表申请内存
	//memset(nand_dev.lut,0,nand_dev.block_totalnum);			//全部清理
    //if(!nand_dev.lut)return 1;				//内存申请失败 
    temp=FTL_CreateLUT(1);
    if(temp) 
    {   
        printf("format nand flash...\r\n");
		temp = f_mkfs(1,1,0);//格式化FLASH,2,盘符;1,不需要引导区,8个扇区为1个簇
...
{
	u8 blktbl[nand_dev.block_totalnum];
	u8 res;
	u32 i,j; 
	u32 goodblock=0;
